<template>
  <div class="dashboard-container">
    ...
    <div style="width: 100%;height: 100%;">
      ...
      <!-- 中间 -->
      <dashcard :contextHeight="'100%'" v-loading="listLoading" class="dashcard-bottom">
          <div slot="context" style="height: 100%;position:absulote">
            ...
            <el-form-item class="page-form-item-userMsg" label="客户姓名:" prop="userId"
                ...
                <div style="width: 215px;height: 54px;border-bottom: 1px solid #cccccc">
                    <span style="font-weight: 400;font-size:22px;font-family: '微软雅黑'">{{userFormData.userName}}</span>
                </div>
            </el-form-item>
            <div style="padding-left: 35px">
              <el-form-item class="page-form-item-userMsg" label="身份证号:" prop="idCardNumber"
                ...
                <div style="width: 195px;height: 54px;">
                    <span style="font-weight: 400;font-size:22px;">{{userFormData.idCardNumber}}</span>
                </div>
            </el-form-item>
            </div>
           </div>
      ...
      <dashcard :contextHeight="'100%'" :bgcolor="'#EEEEEE'" v-loading="listLoading" style="background-color: #ECECEC" class="dashcard-right">
         <div slot="context" style="height: 100%;position:absulote">
            <!-- 系统块,单系统 -->
            <div v-if="sysData.length === 1" style="width:23%;height:156px;display:inline-block;float:left;">
              <div class="sysBtn-box" :key = index v-for="(data, index) in sysData">
                 <div class="sys-btn" style="border-style:solid;border-width:thin;margin-top: 56px;" :class="data.sysName !== customerName?'border-Orange':'border-Blue'">
                    <svg-icon v-if="data.sysName === customerName" class="icon-space" icon-class="centralizedTrading"/>
                    <svg-icon v-if="data.sysName !== customerName" class="icon-space" icon-class="marginTrading"/>
                     <div style="display:inline-block;position:relative;top:-5px">
                       <span class="" :class="data.sysName !== customerName?'orange':'blue'" style="font-size:17px;">{{data.sysName}}</span>

                     </div>
                      <div :class="data.sysName !== customerName?'orange':'blue'" style="font-size:24px;font-weight:bold; float:right;line-height:40px;margin-right:5px">{{data.card.length}}</div>
                 </div>
              </div>
            </div>
            <!-- 多系统展示下面这个 -->
            <div v-if="sysData.length > 1" style="width:23%;height:156px;display:inline-block;float:left;">
              <div class="sysBtn-box" :key = index v-for="(data, index) in sysData">
                 <div class="sys-btn" style="border-style:solid;border-width:thin" :class="data.sysName !== customerName?'orange':'blue'">
                    <svg-icon v-if="data.sysName === customerName" class="icon-space" icon-class="centralizedTrading"/>
                    <svg-icon v-if="data.sysName !== customerName" class="icon-space" icon-class="marginTrading"/>
                     <div style="display:inline-block;position:relative;top:-5px">
                       <span :class="data.sysName !== customerName?'orange':'blue'" style="font-size:17px;">{{data.sysName}}</span>
              ...
              <div class="scroll-card">
                ...
                <!-- 注意slice包前不包后，所以下标取到2 -->
                   <div class="acountMargin"  :key=index v-for="(data, index) in marginTradingData.slice(0, 3)">
                       <!-- 集中交易系统账户块 -->
                       <div v-if="centuralTrading" class="acountCentural-block">
                          <svg-icon class="icon-space" icon-class="ji"/>
                      </div>
                      <div style="height:45px">
                        <span class="marginSpan">客户号-1:</span><span style="font-size:14px">{{data.name}}</span>
                      </div>
                      <div style="height:45px">
                        <span class="marginSpan">所属营业部:</span><span style="font-size:14px">{{data.department}}</span>
                      </div>
                      <!-- 融资融券系统账户块 -->
                      <div v-if="marginTrading" class="acountMargin-block">
      ...
      <!-- 图表部分 -->
      <dashcard :contextHeight="'100%'"  v-loading="listLoading" class="dashcard-bottom-left">
        <div slot="context" style="height: 100%">
          <div style="height: 30px;padding-left:10px">
              <div style="height: 30px;width:100px" class="chart-div" @click="text()">
                ...
                <span style="line-height:30px;font-size: 18px;">资金</span><span style="line-height:30px;font-size: 9px;font-weight: 500">（万元）</span>
              </div>
          </div>
      ...
      <dashcard :contextHeight="'100%'"  v-loading="listLoading" class="dashcard-bottom-right">
         <div slot="context" style="height: 100%">
           <div style="height: 30px;padding-left:10px">
              <div style="height: 30px;width:100px" class="chart-div" @click="text()">
                <svg-icon class="icon-space" style="line-height:30px" icon-class="stock"/>
                <span style="line-height:30px;font-size: 18px;">市值</span><span style="line-height:30px;font-size: 9px;font-weight: 500">（万元）</span>
              </div>
          </div>
          <!-- 多个系统时用柱状图展示 -->
         ...
         <div slot="context" style="height: 100%;">
           <div style="height: 30px;padding-left:10px">
             <div style="height: 30px;width:100px" class="chart-div" @click="text()">
               ...
               <span style="line-height:30px;font-size: 18px;">委托</span><span style="line-height:30px;font-size: 9px;font-weight: 500">（笔）</span>
             </div>
          </div>
         ...
         <div slot="context" style="height: 100%">
           <div style="height: 30px;padding-left:10px">
            <div style="height: 30px;width:80px" class="chart-div" @click="text()">
              ...
              <span style="line-height:30px;font-size: 18px;">成交</span>
            </div>
          </div>
...
export default {
    ...
    created() {
        this.customerName = this.sysData[0].sysName
        this.changeSys(this.sysData)
    },

    methods: {
        changeSys(data) {
            data.forEach((item, i) => {
                if (item.sysName === '集中交易系统') {
                    this.centuralTradingData = item.card
                    this.centuralTrading = true
                    this.marginTrading = false
                } else if (item.sysName === '融资融券交易系统') {
                    this.marginTradingData = item.card
...
.border-Blue{color: #1890FF}
.border-Blue:hover{color: #1890FF}
.dashboard {
  &-container {
    margin: 10px;
